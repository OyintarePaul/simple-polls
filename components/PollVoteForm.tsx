        ...
        <form className="space-y-4" action={action}>
            ...
            <div className="space-y-4">
                {poll.options.map((option) => (
                    <div key={option._id}>
                        <Input type="radio" name="optionID" value={option._id} id={option._id} className="sr-only peer" />
                        <Label htmlFor={option._id} className="cursor-pointer rounded-md ring-2 peer-checked:ring-primary px-4 py-6">{option.optionText}</Label>
                    </div>
                ))}
            </div>
            {state.success ?
