...
export default async function PollDetailsPage({ params }: { params: Promise<{ pollID: string }> }) {
    ...
    await dbConnect()
    const [poll, votes] = await Promise.all([getPollById(pollID), getPollVotes(pollID)])
        ...
        <div className="space-y-4">
            <div className="flex gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">({votes.length} votes)</p>
                </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12">
                    ...
                    {poll.options.map((option) => {
                        ...
                        return (
                            <div className="space-y-1" key={option._id}>
                                ...
                                <p className="text-sm text-muted-foreground">{voteCount} votes</p>
                            </div>
                        )
                    })}
                </div>
                ...
                <div className="flex-1">
                    ...
                    <VotesChart data={votesChartData} /></div>
            </div>
        </div>
    )
}